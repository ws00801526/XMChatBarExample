...
- (void)stop {
    ...
    if (_started) {
        ...
        [self setShouldInterceptTiming:YES];
        _started = NO;
    }
    pthread_mutex_unlock(&_mutex);
...
/// ========================================
/// @name   Private Methodss
/// ========================================

- (void)setShouldInterceptTiming:(BOOL)shouldInterceptTiming {
    if (_startedTime == 0) {
...
- (void)handlePropertyListenerForDefaultOutputDevice {
...
- (NSUInteger)currentTime {
    ...
    double base = [[self class] absoluteTimeConversion] * 1000.0;
...
+ (const AudioObjectPropertyAddress *)propertyListenerAddressForDefaultOutputDevice
{
...
static OSStatus property_listener_default_output_device(AudioObjectID inObjectID,
                                                        ...
                                                        void *inClientData)
{
    __unsafe_unretained XMNAudioRenderer *renderer = (__bridge XMNAudioRenderer *)inClientData;
    [renderer handlePropertyListenerForDefaultOutputDevice];
    return noErr;
}



- (void)setupPropertyListenerForDefaultOutputDevice
{
    AudioObjectAddPropertyListener(kAudioObjectSystemObject,
                                   [[self class] propertyListenerAddressForDefaultOutputDevice],
                                   property_listener_default_output_device,
                                   (__bridge void *)self);
}

- (void)removePropertyListenerForDefaultOutputDevice
{
...
{
    __unsafe_unretained XMNAudioRender *renderer = (__bridge XMNAudioRender *)inRefCon;
    ...
    if (validByteCount < totalBytesToCopy) {
        ...
        [renderer setShouldInterceptTiming:YES];
        ...
        *inActionFlags = kAudioUnitRenderAction_OutputIsSilence;
        ...
        [renderer setShouldInterceptTiming:NO];
    }
...
//    if (analyzers != nil) {
//        for (XMNAudioAnalyzer *analyzer in analyzers) {
