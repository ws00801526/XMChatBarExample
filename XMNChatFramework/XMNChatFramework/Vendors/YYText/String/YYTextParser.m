...
- (BOOL)parseText:(NSMutableAttributedString *)text selectedRange:(NSRangePointer)range {
    ...
    for (NSUInteger i = 0, max = matches.count; i < max; i++) {
        ...
        /** 使用了自定义的添加表情的解析 */
        NSMutableAttributedString *atr = [NSMutableAttributedString yy_attachmentStringWithEmojiImage:emoticon attachmentSize:CGSizeMake(24, 24) alignToFont:[UIFont systemFontOfSize:14.f] alignment:YYTextVerticalAlignmentCenter];
