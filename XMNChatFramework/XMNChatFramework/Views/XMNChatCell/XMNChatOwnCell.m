...
- (void)awakeFromNib {
    ...
    [super awakeFromNib];
    // Initialization code
    [self.backgroundImageView setImage:[XMNCHAT_LOAD_IMAGE(@"message_sender_background_normal") resizableImageWithCapInsets:UIEdgeInsetsMake(30, 16, 16, 24) resizingMode:UIImageResizingModeStretch]];
    [self.backgroundImageView setHighlightedImage:[XMNCHAT_LOAD_IMAGE(@"message_sender_background_highlight") resizableImageWithCapInsets:UIEdgeInsetsMake(30, 16, 16, 24) resizingMode:UIImageResizingModeStretch]];
...
- (void)setupMessageViewConstraint:(XMNMessageType)aMessageType {
    ...
    NSLayoutConstraint *rightConstraint = [NSLayoutConstraint constraintWithItem:self.messageView attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:self.messageContentView attribute:NSLayoutAttributeRight multiplier:1.0 constant:aMessageType == XMNMessageTypeImage ? 0 : -12.f];
